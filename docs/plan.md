...
> 文档状态：需求基线 v0.3.2（最小双源路径 + TechFlow 新闻审计中心）+ v0.2 历史实现证据
...
### 0A.8 TechFlow 新闻审计中心

> **状态：已确认（2026-08-23），并于 2026-08-24 完成本地实现与验证。** 用户要求看到程序实际观察到的每一条 TechFlow 独立快讯，包括没有进入宏观风险观察的快讯；每条快讯都必须展示程序判断、中文理由和 TechFlow 官网核对入口。审计内容进入独立 `/news` 二级页面，不平铺到实时驾驶舱。当前变更尚未提交或推送，远端 CI 未覆盖本次实现。

#### 0A.8.1 产品目的与诚实覆盖边界

新闻审计中心同时回答两个问题：

1. 程序是否仍在按预期读取 TechFlow；
2. 程序对每一条已读取快讯作出了什么判断，为什么采纳、过滤或要求人工复核。

“每一条”严格定义为：**自本功能启用后，被当前 TechFlow 公开列表 adapter 实际观察到的每个独立 `source_item_id` 及其修订版本**。重复轮询同一 ID 不增加新闻行；内容 hash 改变时保留新的 revision。系统不得声称覆盖停机期间从未观察到、已从公开列表移除或因 coverage gap 漏掉的快讯；这些缺口必须作为采集健康事实显示。

第一版不抓取 TechFlow 全站、历史归档或私有接口，不恢复多新闻源聚类、传播链、付费源或 LLM 热路径。现有第 12.7 节“事件中心”中的多源聚类能力属于 v0.2 历史远景；v0.3.2 的页面 2 只实现单一 TechFlow 来源的逐条采集与判断审计。

#### 0A.8.2 每条新闻的确定性判断

每条已观察快讯在首次接收及每次 revision 时，使用当时生效的模型与配置生成不可覆盖的判断记录。判断依次检查：

1. `MACRO_RELEVANCE`：是否属于全球宏观、政策、贸易、地缘、金融稳定、能源供给或加密政策风险；
2. `RISK_DIRECTION`：是否明确表示风险上升；
3. `IMPACT_SEVERITY`：是否达到 `HIGH/SYSTEMIC`；
4. `OBSERVATION_WINDOW`：来源时间与接收时间是否均处于 120 分钟观察窗口。

读者界面只显示以下三个互斥中文结果：

| 结果 | 后台语义 | 使用条件 |
|---|---|---|
| 进入风险观察 | `ENTERED_RISK_OBSERVATION` | 四项检查全部通过；只能武装新闻条件，不能单独卖出 |
| 未进入风险观察 | `NOT_TRIGGERED` | 已能明确证明至少一项不满足 |
| 需要人工复核 | `REVIEW_REQUIRED` | 相关性成立后，方向、严重度或时间证据无法可靠确认 |

每条记录必须包含：完整规范化新闻字段、四项检查、中文摘要理由、`judged_at`、`model_version`、`config_version`、规则版本和观察窗口结束时间。规则升级不得覆盖历史判断；未来新规则重算只能生成独立 replay 结果。

#### 0A.8.3 持久化与内容边界

- 本地私有保存 180 天，使用 append-only、schema 校验和原子清理；运行数据继续由 `.gitignore` 排除；
- 保存标题、最多 600 字必要摘要、TechFlow 详情 URL、原始来源 URL、来源归因、时间、hash、判断和全部 revision；
- 不保存或再分发完整文章正文，不提供正文批量导出；
- 同一 `source_item_id` 在列表页归并为一行，详情中按时间显示 revision 历史；
- 功能启用前的旧 Shadow 日志若缺少标题或判断理由，只能标为“历史信息不完整”，不得伪造回填。

#### 0A.8.4 二级页面信息架构

独立 `/news` 页面采用“新闻编辑部审计台”结构：

1. 顶部采集心跳：最近尝试、最近成功、当前状态、当前页面条数、累计错误和 coverage gap；
2. 结果摘要：全部、进入风险观察、未进入风险观察、需要人工复核；
3. 搜索与互斥筛选；
4. 按首次接收时间倒序的新闻账本；
5. 每行展示时间、标题、中文判断、一句话原因、TechFlow 官网和原始来源链接；
6. 展开后展示四项判断路径、必要摘要、修订历史和审计版本；
7. 首页“最新宏观消息”区域提供“查看全部新闻判断”入口；浏览器刷新、返回和手机窄屏均可用。

首页继续只回答“这一刻要不要操作”，不显示全部新闻、原生枚举、证据 ID 或审计哈希。

#### 0A.8.5 采集存活门禁

TechFlow 正常轮询目标为 10 秒。距最后一次成功语义解析超过 30 秒时，无论最后一次状态是否曾为 `HEALTHY`，当前状态都必须转为过期/延迟：

- 新闻审计页显示“新闻监测延迟”及最后成功时间；
- 正常新闻卖出路径不可确认；
- `NO_NEW_MACRO_ESCALATION` 不得通过，防止把“监测停止”误判为“没有新坏消息”；
- 不沿用旧状态冒充当前正常；
- 恢复成功后重新显示当前状态，并保留累计错误和 gap 事实。

页面心跳与新闻列表必须分开：即使没有新快讯，只要轮询和语义解析持续成功，页面仍能证明采集链正在运行。

#### 0A.8.6 最小只读 API

新增 API 只服务本地客户端审计，不改变外部输入数量：

```text
GET /api/news/audit?outcome=&query=&cursor=&limit=
GET /api/news/audit/:source_item_id
```

列表 API 返回分页后的独立新闻、结果计数、当前 TechFlow 健康和采集指标；详情 API 返回同一新闻的全部 revision。服务端可以保留原生审计字段，默认读者界面必须翻译成自然中文。

#### 0A.8.7 验收标准

- 任一成功解析的独立 TechFlow 快讯都能在 `/news` 找到，包括 `OTHER/UNKNOWN` 和未触发快讯；
- 每条新闻恰有一个最终结果和四项检查，不因被过滤而消失；
- 重复轮询不重复占行，revision 不丢失且历史判断不被覆盖；
- 重启后 180 天范围内的审计记录仍可查询；
- TechFlow 官网链接可核对，缺失原始来源时不虚构；
- 超过 30 秒未成功读取时页面与决策同时降级，恢复后可见；
- 主页面保持简洁，`/news` 在桌面和 390×844 手机视口可用；
- 客户端不直接显示后台原生枚举、来源 ID、证据 ID、hash 或概率化文案；
- 单元、存储、集成、组件和浏览器测试覆盖上述业务结果。
...
> **状态：已确认并部署（2026-08-25）。** 用户选择独立的全云端 24/7 目标，不与旧服务器上的 LetsCash 等项目共用故障域。美国西海岸 SWAS 已完成控制面、应用、公网双源、持久化重启和浏览器读回，状态为 `CLOUD_RUNTIME_VERIFIED_CURRENT`。TLS、异机备份、外部告警和自动 CD 尚未闭环；服务器运行时间也不能倒推为已完成 14 天 Shadow。购买与部署回执分别见 `docs/evidence/2026-08-25-swas-purchase.md`、`docs/evidence/2026-08-25-swas-deployment.md`。
...
**最终选择：** B，全云端 24/7；使用本项目独占的美国西海岸 SWAS。生产 runtime 已部署，本地入口继续用于开发、回放和灾难恢复验证。

**不回复能否继续：** 可以按当前只读边界继续运行和积累 Shadow；新增付费备份、域名/TLS、告警服务或自动 CD 前需按成本与权限单独评审。
