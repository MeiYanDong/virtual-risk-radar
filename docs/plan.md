...
> **状态：已确认（2026-08-25）。** 用户选择独立的全云端 24/7 目标，不与旧服务器上的 LetsCash 等项目共用故障域。美国西海岸 SWAS 已购买并完成控制面读回；当前仅为 `SERVER_PURCHASED / APPLICATION_NOT_DEPLOYED`。在 `P10-003`—`P10-005` 安全与运行门禁完成前，不把服务器存在解释为 Shadow 已连续运行。购买回执见 `docs/evidence/2026-08-25-swas-purchase.md`。
...
**最终选择：** B，全云端 24/7；使用本项目独占的美国西海岸 SWAS。购买不等于部署，迁移前继续保留本地可回放入口。

**不回复能否继续：** 可以继续完成不涉及新秘密或生产切流的部署准备；秘密同步、公开上线和切换 24/7 source of truth 仍需经过对应门禁。
