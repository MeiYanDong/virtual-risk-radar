...
> 文档版本：v0.3.2（最小双源 + TechFlow 新闻审计中心活跃清单；保留 v0.2 历史证据）
...
- [x] **V3-A010** 用户确认按推荐方案建设独立 `/news` 新闻审计页，展示程序实际观察到的全部独立 TechFlow 快讯及逐条判断。
- [x] **V3-A011** 用户确认采用确定性三结果判断、180 天本地私有留存和 30 秒 TechFlow 存活门禁；不增加第二新闻源、付费源或 LLM 热路径。
...
### F2. TechFlow 新闻审计中心（v0.3.2）

- [x] **V3-F2-001** 定义逐条新闻审计 schema：规范化快讯、四项检查、三种最终结果、中文理由、判断时间、规则/模型/配置版本和观察窗口结束时间。
- [x] **V3-F2-002** 实现与 Sell 宏观条件共用的确定性新闻 gate；`OTHER/UNKNOWN`、方向不明、低严重度、过期和合格事件各有互斥判断与真实理由。
- [x] **V3-F2-003** 建立本地私有 append-only 新闻审计 journal，保存标题、最多 600 字摘要、链接、判断及每次 revision；文件权限为 `0600` 且保持 Git ignored。
- [x] **V3-F2-004** 实现 180 天 retention 和原子清理；不得无限保存完整正文，不得把运行数据提交到公开仓库。
- [x] **V3-F2-005** runtime 启动时恢复审计 journal，同一 `source_item_id` 归并最新 revision 进入当前判断，同时保留所有历史 revision。
- [x] **V3-F2-006** 重复轮询同一 hash 不生成重复新闻行；内容改变生成新 revision，旧判断不可覆盖。
- [x] **V3-F2-007** 将 TechFlow freshness 门禁固定为 30 秒并在运行时真实执行；静默停止时转为过期，阻断“无新宏观升级”。
- [x] **V3-F2-008** 为 fetch/body 增加显式截止保护，单次抓取卡住不得永久停止后续轮询；错误与恢复保持 fail-visible。
- [x] **V3-F2-009** 实现 `GET /api/news/audit`：支持结果、关键词、cursor、limit，返回分组新闻、总计、结果计数、source health 和采集指标。
- [x] **V3-F2-010** 实现 `GET /api/news/audit/:source_item_id`：返回该新闻全部 revision，ID 不存在时返回 404，不暴露完整 TechFlow 正文。
- [x] **V3-F2-011** 实现独立 `/news` 二级页面及主页面“查看全部新闻判断”入口；刷新、浏览器返回和直接访问均可用，不引入不必要的页面框架。
- [x] **V3-F2-012** 页面顶部显示中文采集心跳、最后成功、当前列表条数、累计错误与 gap；没有新新闻时仍能判断采集是否运行。
- [x] **V3-F2-013** 新闻列表按时间倒序，支持“全部/进入风险观察/未进入/需复核”和搜索；每行显示标题、中文结论、原因及 TechFlow 官网核对链接。
- [x] **V3-F2-014** 新闻详情展开显示四项中文判断路径、必要摘要、观察窗口和 revision 历史；不直接显示原生枚举、source/evidence ID、hash 或概率。
- [x] **V3-F2-015** 完成桌面和 390×844 响应式设计、键盘操作、屏幕阅读器标签、外链安全属性和清晰的空态/错误态。
- [x] **V3-F2-020** 单元测试覆盖三结果 gate、四项判断、边界时间、未来时间和判断版本不可覆盖。
- [x] **V3-F2-021** 存储测试覆盖 append、幂等、revision、重启恢复、180 天清理、schema/hash 损坏和 `0600` 权限。
- [x] **V3-F2-022** adapter/runtime 测试覆盖 30 秒 stale、显式超时、恢复、旧数据不冒充新鲜以及全部已捕获新闻进入审计链。
- [x] **V3-F2-023** API 集成测试覆盖分页、筛选、搜索、计数、详情、404、健康状态和正文不再分发边界。
- [x] **V3-F2-024** 前端组件与浏览器测试覆盖主页面入口、全部结果、展开详情、官网外链、手机布局和客户端不泄露原生字段。
- [x] **V3-F2-025** 更新 README、runbook、变更记录、OpenAPI 和实现证据；准确声明历史完整性从本功能启用时开始。
